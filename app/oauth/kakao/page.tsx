...
function Component() {
  ...
  const [response, setResponse] = useState<
    | (BaseResponse & Partial<{ data: { description: string; code: string } }>)
    | null
  >(null);

  useEffect(() => {
    if (ignore.current) return;
    api<BaseResponse>(`/oauth/kakao?code=${code}`, {
      method: "POST",
    })
      .then((res) => {
      ...
      .catch(
        (
          err: AxiosError<
            BaseResponse & { data: { description: string; code: string } }
          >
        ) => {
